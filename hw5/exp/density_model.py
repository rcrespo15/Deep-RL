...
class Histogram(Density_Model):
    ...
    def update_count(self, state, increment):
        ...
        self.hist[bin_name] = self.hist[bin_name] + increment
...
class RBF(Density_Model):
    ...
    def fit_data(self, data):
        ...
        self.means = np.asarray(data).reshape(B, ob_dim)
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            for i in range(b):
                deltas[i,:,:] = states[i,:] - self.means
            assert deltas.shape == (b, B, ob_dim)

            # 2. Euclidean distance
            euc_dists = np.zeros((b,B))
            for i in range(b):
                euc_dists[i,:] = (LA.norm(deltas[i,:,:],2,axis=1))**2
            assert euc_dists.shape == (b, B)

            # Gaussian
            gaussians = np.zeros((b,B))
            for i in range(b):
                gaussians[i,:] = np.exp(-euc_dists[i,:]/(2*(self.sigma**2)))
            assert gaussians.shape == (b, B)

            # 4. Average
            densities = np.zeros(b)
            for i in range(b):
                densities[i] = np.sum(gaussians[i,:])#/B
...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        self.elbo = tf.reduce_mean(tf.subtract(self.kl*self.kl_weight,self.log_likelihood)) #add a multiplication
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        """
            ...
            Hint: use build_mlp
        """
        z_mean = build_mlp( state,
                            z_size,
                            scope,
                            n_layers=n_layers,
                            size=hid_size)
        z_logstd = tf.Variable(tf.zeros(z_size), name='z_logstd')
    ...
    def make_prior(self, z_size):
        ...
        """
        prior_mean = tf.zeros(z_size) #is the distribution just zero? for a
        #standard normal
    ...
    def update(self, state1, state2, target):
        ...
        # ll = tf.distributions.Categorical(logits=target).log_prob()
