...
class QLearner(object):
    ...
    def __init__(
        ...
        optimizer_spec,
        session,
        exploration=LinearSchedule(1000000, 5),
        ...
        if double_q==False:
            ...
            if self.done_mask_ph == 1:
               self.y_i = self.rew_t_ph
            else:
               self.y_i = self.rew_t_ph + gamma * self.q_phi_p_max

        else:
            #compute the Q function
            self.Q_phi_ = q_func(obs_tp1_float, self.num_actions, scope="q_func", reuse=True)
            position = tf.argmax(self.Q_phi_,axis=1)
            action = tf.one_hot(position,depth=self.num_actions)
            self.Q_phi_p = q_func(obs_tp1_float, self.num_actions, scope="q_targ_func", reuse=False)
            self.q_phi_p_max = tf.reduce_sum(tf.multiply(self.Q_phi_p, action),axis=1)
            # self.q_phi_p_max = tf.reduce_sum(tf.multiply(self.Q_phi_p, tf.one_hot(self.act_t_ph,depth=self.num_actions)),axis=1)
        ...
        self.log_every_n_steps = 50#10000
    ...
    def step_env(self):
        ...
        if self.model_initialized:
            if random.random() < self.exploration.value(self.t):
                self.generate_action = random.choice(list(range(self.num_actions)))
            else:
                self.generate_action =  self.session.run(self.Q_phi,
                                        feed_dict={self.obs_t_ph:
                                        [self.encoded_observation]})
                self.generate_action = np.argmax(self.generate_action[0])
        else:
            self.generate_action = random.choice(list(range(self.num_actions)))

        #Part5. Call env.step(action) to get obs,reward,done,info
        self.obs, self.reward, self.done, self.info = self.env.step(self.generate_action)

        #Part6. Call store_effect to store variables
        self.replay_buffer.store_effect(self.obs_index,self.generate_action,self.reward,self.done)
